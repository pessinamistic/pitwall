...
fleet_build_launch_cmd() {
  ...
  case "$backend" in
    opencode)
      # Targets fleet-<role>, a distinct mode: primary agent generated by
      # scripts/sync-fleet-agents.mjs (see docs/fleet-mode.md) -- not the
      # bare role name. `opencode run --agent <name>` (confirmed against
      # opencode 1.17.18) requires a primary-mode agent for a top-level
      # invocation; five of the six bare role names are mode: subagent by
      # design (agents/*.md's in-session delegation enforcement -- see
      # README.md, "How orchestration is enforced" -- is deliberately left
      # untouched), so `opencode run --agent <bare role>` silently falls
      # back to the default agent instead of running the intended persona.
      printf '%s run --agent fleet-%s %s %s' \
